...
% 前置部分的页眉页脚设置
% -----------------------------------------------%
% UPDATE: 页眉页脚的配置现在已全部转移至模板
% 正文和后置部分用阿拉伯数字编连续码，前置部分用罗马数字单独编连续码（封面除外）。
% 设置封面页后的页码

%TEST: 使用 \mainmatter 和 \frontmatter 替换这些指令
...
\mainmatter
% \fancyfoot[C]{\zihao{-5} \songti 第 \thepage 页\quad 共\pageref{LastPage} 页} % 所有（奇数和偶数）中间页脚
...
\titleformat{\chapter}{\centering \zihao{3}\heiti}{\thesection}{1em}{}
